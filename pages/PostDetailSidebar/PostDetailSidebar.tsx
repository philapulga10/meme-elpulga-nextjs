...
const PostDetailSidebar: React.FC<PropsType> = ({
  ...
  const handleOnChange = (event) => {
    ...
    if (!isExisting && isCheck) {
      ...
      onChangeDetailForm('category', [...category, value]); // ==> push vào 1 category ==> clone ra 1 array mới
    } else if (!isCheck) {
      // nếu isCheck === false ==> không biết nó tồn tại hay không ==> vẫn remove
      onChangeDetailForm('category', category.filter(id => id !== value)); // chú ý: hàm filter đã làm thay đổi địa chỉ của array gốc vì nó không làm thay đổi array cũ mà return hẳn về array mới
    }
  };
